 ...
 * tracking of two commanded target poses for the two endeffectors. The controller can be reparameterized at
 * runtime via dynamic reconfigure servers.
 */
class MmintDualArmCartesianImpedanceController
 ...
 private:
  ...
  ///< Transformation between world frame and base of panda_1.
  Eigen::Affine3d w_T_Ol_;  // NOLINT (readability-identifier-naming)
  ///< Transformation between world frame and base of panda_2.
  Eigen::Affine3d w_T_Or_;  // NOLINT (readability-identifier-naming)

  ///< Publisher for the tracking frame of the left arm's motion.
  ...
  ///< Rate to trigger publishing the current pose of the tracking frames.
  ...
  /**
   * Callback method that handles updates of the left target pose.
   *
   * @param[in] msg New target pose.
   */
  void leftTargetPoseCallback(const geometry_msgs::PoseStamped::ConstPtr& msg);

  /**
   * Callback method that handles updates of the right target pose.
   *
   * @param[in] msg New target pose.
   */
  void rightTargetPoseCallback(const geometry_msgs::PoseStamped::ConstPtr& msg);

  /**
   * Publishes a Pose Stamped for visualization of the current left pose.
   */
  void publishLeftPose();

  /**
   * Publishes a Pose Stamped for visualization of the current right pose.
   */
