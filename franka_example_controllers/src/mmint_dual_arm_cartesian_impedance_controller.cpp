...
#include <franka_example_controllers/mmint_dual_arm_cartesian_impedance_controller.h>
...
bool MmintDualArmCartesianImpedanceController::initArm(
  ...
  if (model_interface == nullptr) {
    ROS_ERROR_STREAM(
        "MmintDualArmCartesianImpedanceController: Error getting model interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "MmintDualArmCartesianImpedanceController: Exception getting model handle from "
        "interface: "
        << ex.what());
    return false;
  }
  ...
  if (state_interface == nullptr) {
    ROS_ERROR_STREAM(
        "MmintDualArmCartesianImpedanceController: Error getting state interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "MmintDualArmCartesianImpedanceController: Exception getting state handle from "
        "interface: "
        << ex.what());
    return false;
  }
  ...
  if (effort_joint_interface == nullptr) {
    ROS_ERROR_STREAM(
        "MmintDualArmCartesianImpedanceController: Error getting effort joint interface from "
        "hardware");
    return false;
  }
  for (size_t i = 0; i < 7; ++i) {
    try {
      ...
      ROS_ERROR_STREAM(
          "MmintDualArmCartesianImpedanceController: Exception getting joint handles: "
          << ex.what());
      return false;
    }
  }
...
bool MmintDualArmCartesianImpedanceController::init(hardware_interface::RobotHW* robot_hw,
                                                      ros::NodeHandle& node_handle) {
  std::vector<double> cartesian_stiffness_vector;
  std::vector<double> cartesian_damping_vector;

  left_arm_id_ = "panda_1";
  right_arm_id_ = "panda_2";

  std::vector<std::string> left_joint_names;
  if (!node_handle.getParam("left/joint_names", left_joint_names) || left_joint_names.size() != 7) {
    ROS_ERROR(
        "MmintDualArmCartesianImpedanceController: Invalid or no left_joint_names parameters "
        "provided, "
        "aborting controller init!");
    return false;
  }

  boost::function<void(const geometry_msgs::PoseStamped::ConstPtr&)> left_callback =
      boost::bind(&MmintDualArmCartesianImpedanceController::leftTargetPoseCallback, this, _1);

  boost::function<void(const geometry_msgs::PoseStamped::ConstPtr&)> right_callback =
      boost::bind(&MmintDualArmCartesianImpedanceController::rightTargetPoseCallback, this, _1);

  ros::SubscribeOptions left_subscribe_options;
  left_subscribe_options.init("left_frame_target_pose", 1, left_callback);
  left_subscribe_options.transport_hints = ros::TransportHints().reliable().tcpNoDelay();
  sub_target_pose_left_ = node_handle.subscribe(left_subscribe_options);

  ros::SubscribeOptions right_subscribe_options;
  right_subscribe_options.init("right_frame_target_pose", 1, right_callback);
  right_subscribe_options.transport_hints = ros::TransportHints().reliable().tcpNoDelay();
  // sub_target_pose_right_ = node_handle.subscribe(right_subscribe_options);
  ...
  if (!node_handle.getParam("right/joint_names", right_joint_names) ||
    ...
    ROS_ERROR(
        "MmintDualArmCartesianImpedanceController: Invalid or no right_joint_names parameters "
        "provided, "
        "aborting controller init!");
    return false;
  }
  ...
  dynamic_server_compliance_param_->setCallback(boost::bind(
      &MmintDualArmCartesianImpedanceController::complianceParamCallback, this, _1, _2));

  // Get the transformation from base to each EE frame
  tf::StampedTransform w_to_Ol_tf;
  tf::StampedTransform w_to_Or_tf;
  tf::TransformListener listener;
  try {
    if (listener.waitForTransform("base", left_arm_id_ + "_link0", ros::Time(0), ros::Duration(4.0))) {
      listener.lookupTransform("base", left_arm_id_ + "_link0", ros::Time(0), w_to_Ol_tf);
    } else {
      ROS_ERROR(
          "MmintDualArmCartesianImpedanceController: Failed to read transform from base to %s. "
          "Aborting init!",
          ((left_arm_id_ + "_link0").c_str()));
      return false;
    }
    if (listener.waitForTransform("base", right_arm_id_ + "_link0", ros::Time(0), ros::Duration(4.0))) {
      listener.lookupTransform("base", right_arm_id_ + "_link0", ros::Time(0), w_to_Or_tf);
    } else {
      ROS_ERROR(
          "MmintDualArmCartesianImpedanceController: Failed to read transform from base to %s. "
          "Aborting init!",
          ((right_arm_id_ + "_link0").c_str()));
      return false;
    }
  } catch (tf::TransformException& ex) {
    ROS_ERROR("MmintDualArmCartesianImpedanceController: %s", ex.what());
    return false;
  }
  tf::transformTFToEigen(w_to_Ol_tf, w_T_Ol_);  // NOLINT (readability-identifier-naming)
  tf::transformTFToEigen(w_to_Or_tf, w_T_Or_);  // NOLINT (readability-identifier-naming)

  // Setup publisher for the left and right frames.
...
void MmintDualArmCartesianImpedanceController::startingArm(FrankaDataContainer& arm_data) {
...
void MmintDualArmCartesianImpedanceController::starting(const ros::Time& /*time*/) {
  for (auto& arm_data : arms_data_) {
    startingArm(arm_data.second);
  }
}

void MmintDualArmCartesianImpedanceController::update(const ros::Time& /*time*/,
...
void MmintDualArmCartesianImpedanceController::updateArm(FrankaDataContainer& arm_data) {
  // get state variables
...
Eigen::Matrix<double, 7, 1> MmintDualArmCartesianImpedanceController::saturateTorqueRate(
...
void MmintDualArmCartesianImpedanceController::complianceParamCallback(
...
void MmintDualArmCartesianImpedanceController::leftTargetPoseCallback(
    const geometry_msgs::PoseStamped::ConstPtr& msg) {
  try {
    if (msg->header.frame_id != "base") {  // NOLINT
      ROS_ERROR_STREAM(
          "MmintDualArmCartesianImpedanceController: Got pose target with invalid"
          " frame_id "
          << msg->header.frame_id << ". Expected " << "base");
      return;
    }

    // Set target for the left robot.
    auto& left_arm_data = arms_data_.at(left_arm_id_);
    Eigen::Affine3d w_T_EEl{};  // NOLINT (readability-identifier-naming)
    tf::poseMsgToEigen(msg->pose, w_T_EEl);
    Eigen::Affine3d Ol_T_EEl_d = w_T_Ol_.inverse() * w_T_EEl;  // NOLINT (readability-identifier-naming)
    std::cout << "Ol_T_EEl_d" << std::endl;
    std::cout << Ol_T_EEl_d.translation() << std::endl;
    ...
    ROS_ERROR_STREAM("MmintDualArmCartesianImpedanceController: Exception setting left target pose.");
  }
}

void MmintDualArmCartesianImpedanceController::rightTargetPoseCallback(
    const geometry_msgs::PoseStamped::ConstPtr& msg) {
  try {
    if (msg->header.frame_id != "base") {  // NOLINT
      ROS_ERROR_STREAM(
    ...
    auto& right_arm_data = arms_data_.at(right_arm_id_);
    Eigen::Affine3d w_T_EEr{};  // NOLINT (readability-identifier-naming)
    tf::poseMsgToEigen(msg->pose, w_T_EEr);
    Eigen::Affine3d Or_T_EEr_d = w_T_Or_.inverse() * w_T_EEr;  // NOLINT (readability-identifier-naming)
    std::cout << "Or_T_EEr_d" << std::endl;
    std::cout << Or_T_EEr_d.translation() << std::endl;
    ...
    ROS_ERROR_STREAM("MmintDualArmCartesianImpedanceController: Exception setting right target pose.");
  }
}

void MmintDualArmCartesianImpedanceController::publishLeftPose() {
  if (left_frame_pub_.trylock()) {
    ...
    Eigen::Affine3d w_T_EEl = w_T_Ol_ * Ol_T_EEl;   // NOLINT (readability-identifier-naming)
    tf::poseEigenToMsg(w_T_EEl, left_frame_pub_.msg_.pose);
    left_frame_pub_.msg_.header.frame_id = "base";
    left_frame_pub_.unlockAndPublish();
  }
}

void MmintDualArmCartesianImpedanceController::publishRightPose() {
  if (right_frame_pub_.trylock()) {
    ...
    Eigen::Affine3d w_T_EEr = w_T_Or_ * Or_T_EEr;   // NOLINT (readability-identifier-naming)
    tf::poseEigenToMsg(w_T_EEr, right_frame_pub_.msg_.pose);
    right_frame_pub_.msg_.header.frame_id = "base";
...
PLUGINLIB_EXPORT_CLASS(franka_example_controllers::MmintDualArmCartesianImpedanceController,
